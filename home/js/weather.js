const API_KEY =
  typeof window !== "undefined" && window.HOME_API_KEY
    ? window.HOME_API_KEY
    : "";
  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      const response = await fetch(
        `${API_URL}?q=${city}&appid=${API_KEY}&units=${unit}&lang=id`
      );
      ...
      if (response.ok) {
        ...
        result.innerHTML = `<div class="alert alert-danger"><i class="bi bi-exclamation-triangle"></i> ${
...
function displayWeather(data, unit) {
  ...
  result.innerHTML = `
        <div class="text-center">
            <h3><i class="bi bi-geo-alt-fill"></i> ${data.name}, ${
    data.sys.country
  }</h3>
            <div class="weather-icon" style="font-size: 4rem;">${weatherIcon}</div>
            <h1 style="font-size: 3.5rem; font-weight: bold;">${Math.round(
              data.main.temp
            )}${tempUnit}</h1>
...
async function loadPopularCities() {
  ...
  for (const city of provincialCapitals) {
    try {
      const response = await fetch(
        `${API_URL}?q=${city}&appid=${API_KEY}&units=metric&lang=id`
      );
      const data = await response.json();
  ...
  for (let i = 0; i < totalSlides; i++) {
    ...
    for (let j = startIdx; j < endIdx; j++) {
      ...
      slideHTML += `
                <div class="col-md-4">
                    <div class="city-card" onclick="searchCity('${
                      data.name
                    },ID')" style="background-image: url('${weatherBg}');">
